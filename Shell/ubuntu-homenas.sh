...
declare -A SCRIPT_INFO=(
    ...
    ["c15"]="homenas_basic.sh #基础版"
    ["c16"]="homenas_secure.sh #安全版"
...
run_script() {
    ...
    for base_url in "${base_urls[@]}"; do
        ...
        if wget -q --show-progress --timeout=5 "$url" -O "$script_path"; then
            chmod +x "$script_path"
            echo -e "${COLORS[GREEN]}开始执行 $script_name...${COLORS[RESET]}"
            bash "$script_path"
            return
        else
            echo -e "${COLORS[RED]}$short_url下载失败，切换地址重试...${COLORS[RESET]}"
        fi
    done
...
display_submenu() {
    local title="$1"
    local items=(${MENU_ITEMS[$title]})
    echo -e "\n$title"
    echo -e "${COLORS[CYAN]}--------------------------------------------------${COLORS[RESET]}"
    
    for i in "${!items[@]}"; do
        echo -e "${COLORS[WHITE]}$((i + 1))、${SCRIPT_INFO[${items[i]}]#*#}${COLORS[RESET]}"
    done
...
main_menu() {
    ...
    while true; do
        ...
        for i in "${!MENU_ORDER[@]}"; do
            echo -e "${COLORS[WHITE]}$((i + 1))、${MENU_ORDER[i]}${COLORS[RESET]}"
        done
        echo -e "${COLORS[WHITE]}0、退出${COLORS[RESET]}"
        echo -e "${COLORS[CYAN]}==================================================${COLORS[RESET]}"

        read -rp "请选择操作: " choice
        if [[ $choice =~ ^[0-9]+$ ]] && (( choice >= 0 && choice <= ${#MENU_ORDER[@]} )); then
            if (( choice == 0 )); then
                echo -e "${COLORS[GREEN]}退出脚本${COLORS[RESET]}"
                exit 0
            fi
            display_submenu "${MENU_ORDER[choice - 1]}"
        else
            echo -e "${COLORS[RED]}无效选择，请重新输入${COLORS[RESET]}"
        fi
    done
